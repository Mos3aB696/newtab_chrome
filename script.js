    ...
    if (i < startDay || dayNumber >= monthDays) {
      ...
      for (let j = 0; j < daySquare.length; j++) {
        ...
        daySquare[4].firstElementChild.innerText =
          j === 4 ? (dayNumber < 9 ? `0${dayNumber + 1}` : dayNumber + 1) : "";
...
var colorSquareChanged = (task, inputColor) => {
  ...
  let currentItem = document.querySelectorAll(
  ...
  document.getElementById("taskExample").children[task].style.background =
    value;
  ...
  currentTasks = {
    ...
    [task]: {
      color: value,
      label: document.querySelectorAll("#taskLabels input[type='text']")[task]
        .value,
    },
  };
...
var taskNameChanged = (task, inputData) => {
  ...
  let value =
    inputData === undefined ? currentTasks[task].label : inputData.target.value;
  ...
  document.querySelectorAll("#taskLabels input[type='text']")[task].value =
    value;
  let thisMonthData = {
    [selectedMonth]: { tasks: currentTasks, data: current.data },
  };
